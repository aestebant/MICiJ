...
public class HierarchicalSelector extends RouletteSelector {
    ...
    protected void prepareSelection() {
		...
		//primer for para recorrer el array tantas veces como sea su tamaA?o.
        for (int i = 1; i < actsrcsz; i++) {
			// segundo for para recorrer el array y dejar cada vez el valor menor hasta abajo
            for (int k = 0; k < actsrcsz - 1; k++) {
				//realiza la comparaciA?n con su siguiente en el array
                if (FitnessOrdenado[k] > FitnessOrdenado[k + 1]) {
        ...
        for (IIndividual ind : actsrc) {
            ...
            // Calculate
            val = min + (max - min) * ((double) jerarquia / (actsrcsz - 1));
            // Update acc
            acc += val;
            // Set roulette value
            roulette[idx++] = acc;
